...
class BertModel(BertPreTrainedModel):
    ...
    def get_global_local_attention(self, batch_size=3, total_query=160, total_image_embeds=1024, local_visible_query_num=128, device=None):
        # version 1: suppose they are divisible， which means there is no overlap
        ...
        # for multi head attention, add a dimension
        ...
        if encoder_hidden_states is not None:
            ...
            if type(encoder_attention_mask) == list:
                ...
                if local_visible_query_num == 0:    
                    encoder_extended_attention_mask = self.invert_attention_mask(   # 这里变成了 (3,1,1,1024),所以在这里改
                        encoder_attention_mask)
                else:
